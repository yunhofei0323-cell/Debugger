...
def main():
    ...
    with open(
        PROJECT_ROOT / "model" / "keypoint_classifier" / "keypoint_classifier_label.csv",
        encoding="utf-8-sig",
    ) as f:
        keypoint_classifier_labels = [row[0] for row in csv.reader(f)]

    with open(
        PROJECT_ROOT / "model" / "point_history_classifier" / "point_history_classifier_label.csv",
        encoding="utf-8-sig",
    ) as f:
...
def logging_csv(number, mode, landmark_list, point_history_list):
    if mode == 1 and 0 <= number <= 9:
        csv_path = PROJECT_ROOT / "model" / "keypoint_classifier" / "keypoint.csv"
        with open(csv_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])

    if mode == 2 and 0 <= number <= 9:
        csv_path = PROJECT_ROOT / "model" / "point_history_classifier" / "point_history.csv"
        with open(csv_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
